...
const int WINDOW_CELL_WIDTH     = 268;  
const int WINDOW_CELL_HEIGHT    = 268;   
...
int showCells(CellTable &cells, const CellPos &pos,int &heath, int &score) {
    Cell &cell = cells[pos.row][pos.col];
    if (cells[pos.row][pos.col].value = deadiamond_VALUE) {
    	heath -= 2;
    	cells[pos.row][pos.col].state = CELL_DIA2;
	}
	if (cells[pos.row][pos.col].value = bluediamond_VALUE) {
		score += 400;
	cells[pos.row][pos.col].state = CELL_DIA1;		
	}
    if (cells[pos.row][pos.col].value = 0) {
    	cells[pos.row][pos.col].state = CELL_SHOWN;
	}
}
...
void updateGame(Game &game,const SDL_Event &event,  int &heath, int &score) {
    if (game.state != GAME_PLAYING) {
        return;
    }
    ...
    showCells(game.cells,game.lastPos,heath,score);
...
int main(int agrc, char* agrv[]) {
    ...
    while (!quit) {
        ...
        while (SDL_PollEvent(&event) != 0) {
            ...
            updateGame(game,event,heath,score);
        }
    }
